...
class DriController extends BaseController
{
    public function getDris()
    {
        ...
        $driModel->select('dri.dri_id, dri.fullname, area.area_name, section.section_name, division.division_name, department.department_name, dri.created_at')
                 ->join('area', 'area.area_id = dri.area_id', 'left')
                 ->join('section', 'section.section_id = dri.section_id')
                 ->join('division', 'division.division_id = dri.division_id')
                 ->join('department', 'department.department_id = dri.department_id');

        $searchableColumns = ['fullname', 'area.area_name', 'section.section_name', 'division.division_name', 'department.department_name'];
        ...
        $orderColumn = ['dri_id', 'fullname', 'area_name', 'section_name', 'division_name', 'department_name', 'created_at'][$order['column']] ?? 'dri_id';
    ...
    public function addDri()
    {
        ...
        if (empty($userId) || empty($areaId)) {
            return $this->response->setStatusCode(400)->setJSON(['status' => 'error', 'message' => 'Area and User fields are required.']);
        }

        $user = $usersModel->find($userId);
        if (!$user) {
            return $this->response->setStatusCode(404)->setJSON(['status' => 'error', 'message' => 'Selected user not found.']);
        }
    ...
    public function updateDri()
    {
        ...
        if (empty($id) || empty($userId) || empty($areaId)) {
            return $this->response->setStatusCode(400)->setJSON(['status' => 'error', 'message' => 'Area and User fields are required.']);
        }
    ...
    // RESTORED: Helper function to get sections based on division
    public function getSectionsByDivision($divisionId)
    {
        $sectionModel = new SectionModel();
        $sections = $sectionModel->where('division_id', $divisionId)->findAll();
        return $this->response->setJSON(['status' => 'success', 'data' => $sections]);
    }

    public function getDrisBySection($sectionId)
    {
        $driModel = new DriModel();
        $dris = $driModel->where('section_id', $sectionId)->distinct()->findAll();
        return $this->response->setJSON(['status' => 'success', 'data' => $dris]);
    }

    public function getUsersForDropdown()
    {
        $usersModel = new UsersModel();
        $users = $usersModel->select("user_id, CONCAT(firstname, ' ', lastname) as fullname")
                            ->where('status', 'ACTIVE')
