<style>
    .swal-topmost { z-index: 99999 !important; }
    .swal2-toast-container { z-index: 99999 !important; }
    .swal2-textarea { z-index: 99999 !important; }
    .activity-link { text-decoration: none; color: inherit; }
    .activity-link.messages:hover { background-color: #f8f9fa; cursor: pointer; }
    .finding-tag { display: inline-flex; align-items: center; padding: 0.3em 0.6em; }
    .remove-finding-tag { font-weight: bold; margin-left: 6px; line-height: 1; }
    .remove-finding-tag:hover { color: #fff !important; opacity: 0.8; }
    .image-viewport { position: relative; width: 100%; max-width: 350px; height: 260px; border: 1px solid #dee2e6; background-color: #000; margin: 0 auto; display: flex; align-items: center; justify-content: center; overflow: hidden; }
    ...
    #magnifierWindow { position: fixed; top: 20px; right: 20px; width: 400px; height: 400px; border: 3px solid #fff; box-shadow: 0 0 10px rgba(0,0,0,0.5); background-repeat: no-repeat; background-color: #000; display: none; z-index: 99999; will-change: background-position; }
</style>
...
<div class="row">
    <div class="col-lg-8 mb-3">
        <div class="card mb-3">
            <div class="card-header align-items-center"><h4 class="card-title mb-0">Checksheet Forms</h4></div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-sm table-striped table-bordered table-hover nowrap w-100" id="ChecksheetTable">
                        <thead><tr><th>ID</th><th>Area</th><th>Building</th><th>⚠️</th><th>Status</th><th>Actions</th></tr></thead>
                        <tbody></tbody>
                    </table>
                </div>
            </div>
...
<div class="modal fade" id="checksheetModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
        <form id="checksheetForm" class="modal-content">
            <div class="modal-header"><h5 class="modal-title" id="modalTitle">Checksheet Viewer</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><hr>
            <div class="modal-body"><input type="hidden" id="checksheet_id" name="checksheet_id"><div id="checksheetDetailsContainer"></div></div>
            <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button><button type="submit" class="btn btn-primary">Save Entry</button></div>
        </form>
    </div>
</div>
...
<div class="modal fade" id="reviewModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title" id="reviewModalTitle">Review Checksheet</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <table class="table table-bordered table-sm">
                            <tbody><tr><th style="width: 15%;">Building</th><td id="reviewBuilding"></td><th style="width: 15%;">Area</th><td id="reviewArea"></td></tr></tbody>
                        </table>
                    </div>
                </div>
                    ...
                    <table class="table table-bordered table-sm">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>Feedback</th>
                                <th>Action</th>
                                <th class="text-center"><input class="form-check-input" type="checkbox" id="selectAllItems"></th>
                            </tr>
                        </thead>
...
<div class="modal fade" id="activityDetailModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">Verify Corrective Action</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
            <div class="modal-body">
               ...
               <div class="row">
                <div class="col-12">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-sm align-middle">
                            <tbody><tr><th class="w-25">Building</th><td><span id="verifyBuilding"></span></td></tr><tr><th>Area</th><td><span id="verifyArea"></span></td></tr><tr><th>Item</th><td><span id="verifyItem"></span></td></tr><tr><th>Findings</th><td><span id="verifyFindings"></span></td></tr></tbody>
                        </table>
                    </div>
                </div>
                </div>
                <div class="row mt-3 g-4">
                    <div class="col-12 col-lg-7">
                        <div class="row"><div class="col-12 col-md-5 mb-3 mb-md-0 text-center"><h6>Finding Image</h6><div id="verifyFindingImageContainer" class="image-viewport"></div></div><div class="col-md-2 comparison-arrow d-none d-md-flex">&#10140;</div><div class="col-12 col-md-5 text-center"><h6>Action Image</h6><div id="verifyActionImageContainer" class="image-viewport"></div></div></div>
                    </div>
                    <div class="col-12 col-lg-5">
                        <h5>Action Description</h5>
                        <p id="verifyActionDescription" class="p-3 bg-light rounded border" style="min-height: 260px;"></p>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#checksheetDetailsContainer').on('change', '.item-select', function() {
            ...
            if (!itemId) { findingsDropdown.html('').prop('disabled', true).trigger('change'); return; }
            ...
            }).fail(() => findingsDropdown.html('').trigger('change'));
        });
        $('#checksheetDetailsContainer').on('change', '.judgement-toggle', function() {
            ...
            if (isChecked) {
                ...
                findingsDropdown.prop('disabled', true).val(null).trigger('change'); driDropdown.prop('disabled', true).val('');
                ...
                findingsDropdown.prop('disabled', false).trigger('change'); driDropdown.prop('disabled', false);
        ...
        $('#checksheetDetailsContainer').on('click', '.remove-row-btn', function() { const row = $(this).closest('.row-item'); row.find('.findings-select').select2('destroy'); row.remove(); });
        ...
        $('#approveActivityBtn').on('click', function() {
            ...
            $.ajax({
                ...
                data: {
                    checksheet_id: checksheetId,
                    data_ids: [dataId], // Send as an array with one item
                    '<?= csrf_token() ?>': '<?= csrf_hash() ?>'
                },
                dataType: 'json',
                success: function(res) {
                    if (res.status === 'success') {
                        ...
                        Swal.fire('Approved!', res.message, 'success');
                        checksheetTable.ajax.reload();
                        loadChecksheetActivity(1, ''); // Refresh activity feed
                    } else {
                        Swal.fire('Error!', res.message || 'An error occurred.', 'error');
                    }
                },
    ...
    function addRow(items = [], dris = []) {
        const rowsContainer = $('#findingsRowsContainer'), rowIndex = rowsContainer.children('tr').length;
        ...
        const newRowHtml = `<tr class="row-item"><td><select class="form-select form-select-sm item-select" name="item_id[]" required><option value="" selected disabled>Choose Item...</option>${itemOptions}</select><input type="hidden" class="control-hidden-field" name="control[]" value=""></td><td><input type="text" class="form-control form-control-sm sub-control-field" name="sub_control[]" placeholder="Sub-control" readonly></td><td><div class="form-check form-switch d-flex justify-content-center"><input class="form-check-input judgement-toggle" type="checkbox" role="switch" checked><label class="form-check-label ms-2 text-success">OK</label><input type="hidden" name="status[]" value="1"></div></td><td><select class="form-select form-select-sm findings-select" name="findings_id[${rowIndex}][]" multiple="multiple" disabled></select></td><td><div class="image-upload-container"><label class="btn btn-sm btn-outline-secondary file-input-label w-100"><i class='bx bx-paperclip'></i> Attach<input type="file" class="finding-image-input" name="finding_image[]" accept="image/png, image/jpeg, image/jpg" capture="environment" style="display: none;"></label><div class="image-preview-container align-items-center" style="display: none;"><img src="" alt="Preview" class="img-thumbnail me-2" style="max-width: 80px; max-height: 40px;"><button type="button" class="btn btn-danger btn-sm remove-image-btn">&times;</button></div></div></td><td><select class="form-select form-select-sm" name="dri_id[]" required><option value="" selected disabled>Choose DRI...</option>${driOptions}</select></td><td><input type="text" class="form-control form-control-sm" name="remarks[]" placeholder="Remarks..."></td><td><div class="form-check form-switch d-flex justify-content-center"><input class="form-check-input priority-toggle" type="checkbox" role="switch"><input type="hidden" name="priority[]" value="0"></div></td><td class="text-end"><button type="button" class="btn btn-danger btn-sm remove-row-btn"><i class='bx bx-trash'></i></button></td></tr>`;
        const newRow = $(newRowHtml).appendTo(rowsContainer);
        newRow.find('.findings-select').select2({ theme: 'bootstrap-5', width: '100%', placeholder: 'Select item first...', dropdownParent: $('#checksheetModal') });
    ...
    // ✅ UPDATED: Function completely overhauled to meet all new requirements
    function reviewRequest(id) {
        $.getJSON(`<?= base_url('checksheet/getChecksheetReviewData') ?>/${id}`, function(response) {
            ...
            let hasVerifiableItems = false; // Flag to check if there are items with status = 1
            ...
            // Disable "Approve Selected" button if no items have status = 1
